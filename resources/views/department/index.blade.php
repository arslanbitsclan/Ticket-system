	...
	<div id="kt_app_content_container" class="app-container container-fluid">
		...
		<div class="card">
			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<!--end::Svg Icon-->
						<input type="text" data-kt-department-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Department" />
					</div>
					<!--end::Search-->
				</div>
				<!--begin::Card title-->
			...
			<div class="card-body pt-0">
				<!--begin::Table-->
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="department_table">
					<!--begin::Table head-->
					<thead>
						<!--begin::Table row-->
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
							...
							<th class="min-w-125px  @if(!auth()->user()->can('edit-department') && !auth()->user()->can('delete-department') ) text-end  @endif">Created Date</th>
...
<script>
      $(document).ready(function () {
		...
		try {
			...
			let data_table = $('#department_table').DataTable({
				serverSide: true,
				stateSave: false,
				ajax: {
		...
		$(document).on("click", ".delete", function(){
			...
			Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
						url: "{{ route('department.delete-ajax')}}",
						type: "POST",
						data:  {id: id},
