			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<!--end::Svg Icon-->
						<input type="text" data-kt-permission-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Permission" />
					</div>
					<!--end::Search-->
				</div>
				<!--begin::Card title-->
				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="permission_table">
					...
					<thead>
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
							...
							<th class="min-w-125px  @if(!auth()->user()->can('delete-permission') ) text-end  @endif">Created Date</th>
...
<script>
      $(document).ready(function () {
		...
		try {
			...
			let data_table = $('#permission_table').DataTable({
				serverSide: true,
				stateSave: false,
				ajax: {
		...
		$(document).on("click", ".delete", function(){
			...
			Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
						url: "{{ route('permission.delete-ajax')}}",
						type: "POST",
						data:  {id: id},
