			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<!--end::Svg Icon-->
						<input type="text" data-kt-contact-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Contact" />
					</div>
					<!--end::Search-->
				</div>
				<!--begin::Card title-->
				...
				<div class="card-toolbar">
					...
					<div class="d-flex justify-content-end" data-kt-contact-table-toolbar="base">
						@can('add-contact')
						<!--begin::Add contact-->
						<a href="{{ route('contact.create') }}" class="btn btn-primary">Create a new contact</a>
						<!--end::Add contact-->
						@endcan
					</div>
					<!--end::Toolbar-->
				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="contact_table">
					...
					<thead>
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
							...
							<th class="min-w-125px  @if(!auth()->user()->can('edit-contact') && !auth()->user()->can('delete-contact') ) text-end  @endif">Created Date</th>
...
<script>
      $(document).ready(function () {
		...
		try {
			const filterSearch = document.querySelector('[data-kt-contact-table-filter="search"]');
			...
			let data_table = $('#contact_table').DataTable({
				serverSide: true,
				stateSave: false,
				ajax: {
					...
					error: function (xhr, error, thrown) {
						if (xhr.status == 419) { //If contact is loggedout due to inactivity, then reload page
							location.reload();
						}
						else {
							Swal.fire({
