...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
	...
	<div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
		...
		<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
			<!--begin::Title-->
			<h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">{{ $contact->first_name.' '.$contact->last_name }}</h1>
			<!--end::Title-->
			<!--begin::Breadcrumb-->
		...
		<div class="card mb-5 mb-xl-10">
            <div class="card-header border-0">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">{{ $contact->first_name.' '.$contact->last_name }}</h3>
                </div>
            </div>
            <form class="form" action="{{ route('contact.update',$contact->id) }}" method="POST" id="kt_modal_add_contact_form">
                @csrf
                @method('PATCH')
                <div class="card-body border-top p-9">
                    <div class="row g-9 mb-7">
                        <div class="col-md-6 fv-row">
                            ...
                            <input class="form-control form-control-solid" type="text" placeholder="Please enter first name" name="first_name" id="first_name" value="{{ old('first_name') ?? $contact->first_name }}"/>
                        </div>
                        
                         <div class="col-md-6 fv-row">
                            <label class="required fs-6 fw-semibold mb-2" for="last_name">Last name</label>
                            <input class="form-control form-control-solid" type="text" placeholder="Please enter last name" name="last_name" id="last_name" value="{{ old('last_name') ?? $contact->last_name }}"/>
                        </div>
                        
                        <div class="col-md-6 fv-row">
                            <label class="required fs-6 fw-semibold mb-2" for="email">Email</label>
                            <input class="form-control form-control-solid" type="email" placeholder="Please enter email" name="email" id="email" value="{{ old('email') ?? $contact->email }}"/>
                        </div>
                    
                        
                        <div class="col-md-6 fv-row">
                            <label class="required fs-6 fw-semibold mb-2" for="phone">Phone</label>
                            <input class="form-control form-control-solid" type="number" placeholder="Please enter phone number" name="phone" id="phone" value="{{ old('phone') ?? $contact->phone }}"/>
                        </div>
                        ...
                        <div class="col-md-6 fv-row">
                            ...
                            <input class="form-control form-control-solid" type="text" placeholder="Please enter address" name="address" id="address" value="{{ old('address') ?? $contact->address }}"/>
                        </div>
                        
                        <div class="col-md-6 fv-row">
                            <label class="required fs-6 fw-semibold mb-2" for="city">City</label>
                            <input class="form-control form-control-solid" type="text" placeholder="Please enter city" name="city" id="city" value="{{ old('city') ?? $contact->city }}"/>
                        </div>
                        ...
                        <div class="col-md-6 fv-row">
                            ...
                            <select aria-label="Select a country" data-control="select2"  data-placeholder="Select a country"  id="country" name="country" class="form-select form-select-solid fw-bold">
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ ($contact->country  == $country->id) ? 'selected' :'' }}> {{ $country->name }} </option>
                                @endforeach
