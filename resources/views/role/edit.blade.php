...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
	...
	<div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
		...
		<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
			<!--begin::Title-->
			<h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">{{ $role->name }}</h1>
			<!--end::Title-->
			<!--begin::Breadcrumb-->
		...
		<div class="card mb-5 mb-xl-10">
            <div class="card-header border-0">
                <div class="card-title m-0">
                    <h3 class="fw-bold m-0">{{ $role->name }}</h3>
                </div>
            </div>
            <form class="form" action="{{ route('role.update',$role->id) }}" method="POST" id="kt_modal_add_role_form">
                @csrf
                @method('PATCH')
                <div class="card-body border-top p-9">
                    <div class="row g-9 mb-7">
                        <div class="col-md-6 fv-row">
                            ...
                            <input class="form-control form-control-solid" type="text" placeholder="Please enter role name" name="name" id="name"  value="{{ old('name') ?? @$role->name}}"/>
                        </div>
                    </div>
                            ...
                            @foreach($permissions as $permission)
                                <div class="col-md-3 fv-row">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="permission[{{$permission->name}}]" id="{{$permission->name}}" {{ in_array($permission->name, $role_permissions) ? 'checked' : ''}} />
