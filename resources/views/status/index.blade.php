			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<!--end::Svg Icon-->
						<input type="text" data-kt-status-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Status" />
					</div>
					<!--end::Search-->
				</div>
				<!--begin::Card title-->
				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="status_table">
					...
					<thead>
						...
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
							...
							<th class="min-w-125px  @if(!auth()->user()->can('edit-status') && !auth()->user()->can('delete-status') ) text-end  @endif">Created Date</th>
...
<script>
      $(document).ready(function () {
		...
		try {
			const filterSearch = document.querySelector('[data-kt-status-table-filter="search"]');
			...
			let data_table = $('#status_table').DataTable({
				serverSide: true,
				stateSave: false,
				ajax: {
