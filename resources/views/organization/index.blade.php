	...
	<div id="kt_app_content_container" class="app-container container-fluid">
		...
		<div class="card">
			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<!--end::Svg Icon-->
						<input type="text" data-kt-organization-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Search Organization" />
					</div>
					<!--end::Search-->
				</div>
				<!--begin::Card title-->
				...
				<div class="card-toolbar">
					<!--begin::Toolbar-->
					<div class="d-flex justify-content-end" data-kt-organization-table-toolbar="base">
			...
			<div class="card-body pt-0">
				<!--begin::Table-->
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="organization_table">
					<!--begin::Table head-->
					<thead>
						<!--begin::Table row-->
						<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
							...
							<th class="min-w-125px  @if(!auth()->user()->can('edit-organization') && !auth()->user()->can('delete-organization') ) text-end  @endif">Created Date</th>
...
<script>
      $(document).ready(function () {
		...
		try {
			const filterSearch = document.querySelector('[data-kt-organization-table-filter="search"]');
			...
			let data_table = $('#organization_table').DataTable({
				serverSide: true,
				stateSave: false,
				ajax: {
					url: "{{ route('organization.fetch') }}",
					data: data,
					type: "post",
					error: function (xhr, error, thrown) {
						if (xhr.status == 419) { //If Organization is loggedout due to inactivity, then reload page
							location.reload();
						}
						else {
							Swal.fire({
